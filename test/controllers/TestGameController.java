...
class TestGameController {
	...
	public class GameControllerTest {
	 ...
	 // Helper method to check if the game loop was executed
	  ...
	  //The assertDoesNotThrow() method ensures that the method runs without encountering any unhandled exceptions.
	    ...
	    //chooses to view instructions ('i' option) and then starts the game ('p' option) afterwards.
	    //The assertDoesNotThrow() method ensures that the method runs without encountering any unhandled exceptions.
...
//This test verifies the behavior of the startGame() method when the user provides invalid inputs followed by a 
//valid input ('p' option) to start the game. 
	  //The assertDoesNotThrow() method ensures that the method runs without encountering any unhandled exceptions.
	    
	    @Test
	    void testStartGame_GameStartWithInvalidInputFollowedByValidInput() throws IOException {
	    ...
	    //This test focuses on ensuring that the startGame() method behaves correctly when the user 
	    //provides multiple invalid choices before finally starting the game.
	    //The assertDoesNotThrow() method ensures that the method runs without encountering any unhandled exceptions.
	    
	    @Test
	    public void testStartGame_MultipleInvalidChoicesFollowedByGameStart() throws IOException {
	        String simulatedInput = "invalid1\ninvalid2\ninvalid3\np\n";
	        InputStream inputStream = new ByteArrayInputStream(simulatedInput.getBytes());
	        System.setIn(inputStream);
	   ...
	   @Test
	    void testStartGame_GameStartWithNoInputFollowedByValidInput() throws IOException {
	 ...
	 //The assertDoesNotThrow() method ensures that the method runs without encountering any unhandled exceptions.
	   ...
	   @Test
	    void testGameLoop_NormalFlow() throws IOException {
	        String simulatedInput = "p\ns\nr\n"; // Simulated input: start game, show resources, roll dice
	        InputStream inputStream = new ByteArrayInputStream(simulatedInput.getBytes());
	        System.setIn(inputStream);
	        ...
	        // Assuming the game loop continues until a game-ending condition is met which 
	        //we need to create a method for that!!! more tests can be created when we do this
	        // We cannot directly test the game loop here as it's a continuous loop until the game ends
	      
	  
	   //this test verifies that the gameLoop() method behaves correctly when a player
	   //chooses to roll the dice during their turn
	         }
	 @Test
	    void  //do we need add more checks in game loop ie for the players next turn etc
