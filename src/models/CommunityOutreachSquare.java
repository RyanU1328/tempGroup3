...
public class CommunityOutreachSquare extends Square {
    ...
    public void performAction(Player player, Scanner scanner) {
        ...
        if ("invest".equalsIgnoreCase(choice)) {
            if (player.getResources() >= investmentOption) {
                player.deductResources(investmentOption);
                System.out.println(player.getName() + " chose to invest in the community, resources now: " + player.getResources());
            } else {
                System.out.println("Not enough resources to invest.");
            }
        } else if ("receive".equalsIgnoreCase(choice)) {
            player.addResources(donationOption);
            System.out.println(player.getName() + " chose to receive a donation, resources now: " + player.getResources());
        }
    }
}
